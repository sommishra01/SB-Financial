...
/** Format a number into Indian currency shorthand */
function fmt(n) {
...
/** Format without shorthand (for smaller values) */
function fmtPlain(n) {
  return '₹' + Math.round(n).toLocaleString('en-IN');
}

/** Sync the slider track gradient fill percentage */
...
/** Attach sync on input for every range slider on the page */
...
/* ============================================================
   DONUT CHART HELPER
   ============================================================ */
const CIRC = 238.76; // circumference of r=38 circle

/**
 * Draw two arcs: invested (teal/white) and returns (yellow).
 * @param {string} investedId  - SVG circle element id for invested arc
 * @param {string} returnsId   - SVG circle element id for returns arc
 * @param {string} pctLabelId  - Element id to show returns %
 * @param {number} invested    - total amount invested
 * @param {number} corpus      - total corpus
 */
...
function calcRetirement() {
  ...
  // Inflate monthly expenses to retirement date
  const futMonthlyExp = exp * Math.pow(1 + inf, yrsToRet);

  // Corpus needed: using perpetuity / 25x rule when real return <= 0
  ...
  // Animate timeline fill proportional to years worked vs total lifespan proxy (80yrs)
...
function calcGoal() {
  ...
  // Future value of existing savings (compounded annually for simplicity)
  const fvSaved    = saved > 0 ? saved * Math.pow(1 + rate, yrs) : 0;
  const remaining  = Math.max(goal - fvSaved, 0);

  // Monthly SIP to cover remaining target
  ...
  // Progress bar: how much is already covered by savings
